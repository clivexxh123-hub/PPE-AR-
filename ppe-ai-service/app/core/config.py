...
class Settings(BaseModel):
    ...
    comfyui_human_wearing_workflow_path: Path = Path(
        os.getenv(
            "COMFYUI_HUMAN_WEARING_WORKFLOW_PATH",
            str(base_dir / "app" / "templates" / "comfyui" / "human_wearing_inpaint_workflow.json"),
        )
    )
    comfyui_image_node_id: str | None = os.getenv("COMFYUI_IMAGE_NODE_ID")
    comfyui_mask_node_id: str | None = os.getenv("COMFYUI_MASK_NODE_ID", "12")
    ...
    # Only the PPE/body contact band is repainted in human_wearing mode.
    comfyui_human_wearing_blend_denoise: float = float(
        os.getenv("COMFYUI_HUMAN_WEARING_BLEND_DENOISE", "0.65")
    )
