...
def render_human_wearing_design(
    ...
    *,
    ...
    anchors = analyze_body_anchors(human, view=view, framing=framing)
    ...
    for requested in requested_placements:
        ...
        if normalized_category == "vest":
            layer = _warp_vest_rows(ppe, target_width, target_height, view)
        else:
            layer = ppe.resize((target_width, target_height), Image.Resampling.LANCZOS)
        if requested.get("mirror"):
