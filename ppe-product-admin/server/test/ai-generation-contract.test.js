...
function businessPayload(overrides = {}) {
    return {
        ...
        scene: {
            ...
            name: "建筑工地",
            industry: "建筑",
            image: "/uploads/scenes/construction.png"
        },
...
test("a multi-product task keeps every selected PPE item and the scene reference", () => {
        ...
        outfitItems: [
            ...
            },
            {
                product: {
                    ...
                    category_level_2: "手部防护"
                },
                view: { id: "front", image: "/uploads/products/gloves.png" },
                ppeCategory: "gloves"
            }
        ],
        composition: { view: "front", framing: "full_body" }
    }));
    assert.equal(prepared.outfitItems.length, 3);
    assert.equal(prepared.task.parameters.outfit_items.length, 3);
    assert.deepEqual(
