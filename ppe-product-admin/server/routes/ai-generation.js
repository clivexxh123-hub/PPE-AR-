...
function normalizeOutfitItems(body, composition) {
    const rawItems = Array.isArray(body?.outfitItems) && body.outfitItems.length
        ? body.outfitItems
        : [{
            product: body?.product,
            view: body?.view,
            logo: body?.logo,
            ppeCategory: body?.ppeCategory
        }];
    if (rawItems.length > 6) {
        const error = new Error("单次人物穿戴最多支持 6 件 PPE 产品");
        error.statusCode = 400;
        throw error;
    }
    ...
    return rawItems.map((rawItem, index) => {
        ...
        const view = rawItem?.view || {};
        const logo = rawItem?.logo || view?.logo || {};
        ...
        if (!productImage) {
            const error = new Error(`第 ${index + 1} 件产品没有可供 AI 使用的图片`);
            error.statusCode = 400;
            throw error;
        }
        if (ppeCategory === "unknown") {
            const error = new Error(`第 ${index + 1} 件产品无法识别 PPE 穿戴类别`);
            error.statusCode = 400;
            throw error;
        }
        if (["gloves", "boots"].includes(ppeCategory) && composition?.framing !== "full_body") {
            const error = new Error(`${ppeCategory === "gloves" ? "手套" : "鞋子"}只能使用全身构图生成`);
            error.statusCode = 400;
            throw error;
        }
        return {
...
function buildBusinessTask(body, actor = null) {
    ...
    if (generationMode === "human_wearing" && !sceneImage) {
        const error = new Error("人物穿戴模式必须选择带图片的行业场景");
        error.statusCode = 400;
        throw error;
    }
    ...
    const parameters = {
        product_name: outfitItems.map((item) => item.productName).join("、"),
        product_category: [...new Set(outfitItems.map((item) => item.productCategory))].join("、"),
        ...
        ...(generationMode === "human_wearing" ? {
            outfit_items: outfitItems.map((item) => ({
                product_id: asTrimmedText(item.product.id, `local-${item.ppeCategory}`),
                product_name: item.productName,
                product_category: item.productCategory,
                product_code: asTrimmedText(item.product.goods_no),
                product_view: asTrimmedText(item.view.id, "front"),
        ...
        prompt_overrides: {
            ...
            outfit_summary: outfitItems.map((item) => `${item.productName}(${item.ppeCategory})`).join("；"),
...
async function prepareTaskSources(prepared) {
    ...
    const materializeOnce = async (value) => {
        if (!sourceCache.has(value)) sourceCache.set(value, materializeImageSource(value));
        return sourceCache.get(value);
    };
    const productSource = await materializeOnce(prepared.productImage);
    ...
    if (prepared.logoImage) {
        const logoSource = await materializeOnce(prepared.logoImage);
    ...
    if (prepared.generationMode === "human_wearing") {
        const humanSource = await materializeOnce(prepared.humanImage);
        const sceneSource = await materializeOnce(prepared.sceneImage);
        ...
        prepared.task.parameters.scene_reference = sceneSource.imageSource;
        prepared.task.parameters.outfit_items = await Promise.all(prepared.outfitItems.map(async (item) => {
            const itemProduct = await materializeOnce(item.productImage);
            const itemLogo = item.logoImage ? await materializeOnce(item.logoImage) : null;
            return {
                product_id: asTrimmedText(item.product.id, `local-${item.ppeCategory}`),
                product_name: item.productName,
                product_category: item.productCategory,
                ...
                ...(itemLogo ? { logo_image: itemLogo.imageSource } : {})
            };
        }));
    }
...
module.exports.normalizeOutfitItems = normalizeOutfitItems;
