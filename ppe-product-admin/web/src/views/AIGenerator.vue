...
const compositionOptions = computed(() => compositionsForProduct(selectedProduct.value));
...
async function runGenerationPreflight(productView) {
  const checks = [];
  ...
  try {
    const product = await inspectTransparentPng(productView.image);
    if (!product.isPng) {
      add("product", "透明 PPE 产品图", "failed", "文件内容不是 PNG；不能只依赖 .png 文件名");
    } else if (!product.hasTransparency || product.alpha.transparentRatio < 0.01) {
      add("product", "透明 PPE 产品图", "failed", `PNG ${product.width}×${product.height} 的有效透明区域不足 1%`);
    } else if (product.alpha.visibleRatio < 0.01) {
      add("product", "透明 PPE 产品图", "failed", `PNG ${product.width}×${product.height} 几乎没有可见产品主体`);
    } else {
      add(
        "product",
        "透明 PPE 产品图",
        "passed",
        `PNG ${product.width}×${product.height}，透明区域约 ${Math.round(product.alpha.transparentRatio * 100)}%`
      );
    }
  } catch (error) {
    add("product", "透明 PPE 产品图", "failed", error?.message || "产品图检查失败");
  }

  const ppeCategory = ppeCategoryForProduct(selectedProduct.value);
  if (ppeCategory === "unknown") {
    add("ppe-category", "PPE 穿戴类别", "failed", "无法识别产品属于帽子、背心、护目镜、手套或鞋子");
  } else if (["gloves", "boots"].includes(ppeCategory)) {
    add("ppe-category", "PPE 穿戴类别", "passed", `${ppeCategory === "gloves" ? "手套" : "鞋子"}仅生成双手/双脚完整可见的全身构图`);
  } else {
    add("ppe-category", "PPE 穿戴类别", "passed", `已识别类别：${ppeCategory}`);
  }
  ...
  if (selectedScene.value?.name || selectedScene.value?.scene_name) {
    add("scene", "行业场景", "warning", "当前仅进入受控 Prompt，尚未作为场景参考图上传");
  } else {
    add("scene", "行业场景", "failed", "未选择行业场景");
  }

  const logo = logoForView(productView);
  if (logo?.image) {
    try {
      const decoded = await verifyDecodableImage(logo.image, "Logo");
      add("logo", "Logo", "passed", `图片可读取：${decoded.width}×${decoded.height}`);
    } catch (error) {
      add("logo", "Logo", "failed", error?.message || "Logo 检查失败");
    }
  } else {
    add("logo", "Logo", "warning", "本次未选择 Logo；允许继续，但不会产生品牌印刷效果");
  }
...
function generationPayload(view, composition) {
  ...
  return {
    ...
    ppeCategory: ppeCategoryForProduct(selectedProduct.value),
...
async function runComposition(composition, productView) {
  ...
  try {
    const created = await request.post(
      "/ai/generations",
      generationPayload(productView, composition),
...
async function startAIGeneration() {
  ...
  const productView = selectedProductViews.value.find(view => view.id === "front" && view.image)
    || selectedProductViews.value.find(view => view.image);
  if (!productView) {
    generationError.value = "请至少选择一个带产品图片的生成视图";
    return;
  }
  ...
  generationSourceProductId.value = String(selectedProduct.value?.id || "");
  ...
  try {
    ...
    await runGenerationPreflight(productView);
    ...
    for (let index = 0; index < compositionOptions.value.length; index += 1) {
      ...
      if (await runComposition(composition, productView)) succeeded += 1;
    }
...
async function retryComposition(result) {
  ...
  if (!productView || !compositionModel?.image || !selectedScene.value) {
  ...
  try {
    ...
    await runGenerationPreflight(productView);
    await runComposition(result, productView);
  } catch (error) {
    generationError.value = errorMessage(error);
  } finally {
    generationProgressText.value = "";
