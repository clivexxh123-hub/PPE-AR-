...
export function compositionsForProducts(products = []) {
  const categories = products.map(ppeCategoryForProduct);
  if (categories.some(category => ["gloves", "boots"].includes(category))) {
    return HUMAN_COMPOSITION_OPTIONS.filter(option => option.framing === "full_body");
  }
  return HUMAN_COMPOSITION_OPTIONS;
}
